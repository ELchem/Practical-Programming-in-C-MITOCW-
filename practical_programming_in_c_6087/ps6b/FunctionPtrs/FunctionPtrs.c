...
{
	char * fna = ((struct student *)a)->fname;
	char * fnb = ((struct student *)b)->fname;
	int len_fna = strlen(fna);
	int len_fnb = strlen(fnb);
	int short_fn;
    ...
    return (-1 * (len_fna - len_fnb)); /*return positive value for shortest string, 0 if they are the same*/
}
...
{
	char * lna = ((struct student *)a)->lname;
	char * lnb = ((struct student *)b)->lname;
	int len_lna = strlen(lna);
	int len_lnb = strlen(lnb);
	int short_ln;
	...
	/*return positive value for shortest string, 0 if they are the same*/
	return (-1 * (len_lna - len_lnb));
}
...
{
	int age = ((struct student *)prec)->age;
	int cutoff = *((int *)arg);
	if (age > cutoff)
	{
		printrec(prec,arg);
